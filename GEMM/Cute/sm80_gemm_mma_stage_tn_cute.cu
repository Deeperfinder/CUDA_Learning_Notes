...
gemm_multi_stage(void *Dptr, const void *Aptr, const void *Bptr, int m, int n,
  ...
  auto tCrA = thr_mma.partition_fragment_A(gA(_, _, 0));  // (MMA, MMA_M, MMA_K) MMA_M表示在kTileM下需要多少次MMA才能完成
  ...
  // Global mem to shared mem 32x4 8
  G2SCopyA g2s_tiled_copy_a;
  auto g2s_thr_copy_a = g2s_tiled_copy_a.get_slice(idx); // 获取该线程负责的线程搬运切片
...
#pragma unroll
  // thread layout 32x4, stride 4; val layout 8
  for (int istage = 0; istage < kStage - 1; ++istage ) {
    ...
    /*
    PTX 有两种异步拷贝操作完成机制
    Async-group机制和基于mbarrier的机制
    */
    cp_async_fence();  // 提交异步加载操作 
  ...
  // 这里的数字为容忍多少个cp.async-group未完成，这里有两个操作数一个stage
  cp_async_wait<kStage - 2>();  
  //插入同步点，cp.async 操作执行的写入数据才对执行线程可见，此时为了对 block 内其他线程可见，需要在wait操作后加一个同步点
  __syncthreads(); 
  int ik = 0;
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      // STAGE 1
      // 使用在上一次ik迭代中加载好的数据 (MMA, MMA_M, ik)
...
namespace config {
  ...
  struct GemmConfig {
    ...
    using MMA = decltype(make_tiled_mma(mma_atom{}, MMA_EU_RepeatT{}, MMA_P_T{}));
      // 异步拷贝指令
      // 32 x 4 threads, 每个threads copy 1x8个values（128bits） 
      // threads values layout map coordinates to thr_idx and val_idx
    using g2s_copy_op = SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>; //  原子操作， 实现全局内存到共享内存异步拷贝，数据只在L2做cache，L1做bypass
    using g2s_copy_traits = Copy_Traits<g2s_copy_op>; // // 描述拷贝操作的能力、对齐、向量化等元信息
    ...
    using SmemLayoutAtomC = decltype(composition(
        Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}),  //32*32
...
int main(int argc, char *argv[]) {
  ...
  printf("[log] min_time = %12.8lf s , avg_time = %12.8lf, maßx_time =  %12.8lf s, \n", min_sec, avg_sec, max_sec);
